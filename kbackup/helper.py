...
class GetDeploymentAssociations:
    ...
    def get_associated_ingress(self, service: Optional[str], namespace: str) -> Optional[str]:
        if not service:
            return None
    ...
    def get_associated_secretprovider(self, deployment) -> Optional[List[Dict[str, str]]]:
        """
        ...
        for volume in volumes:
            if getattr(volume, 'csi', None) is not None:
                ...
                if getattr(csi, 'driver', None) == 'secrets-store.csi.k8s.io':
                    ...
                    spc_name = None
                    if getattr(csi, 'volume_attributes', None) and 'secretProviderClass' in csi.volume_attributes:
                        spc_name = csi.volume_attributes['secretProviderClass']
                    
                    result.append({
                        'volume_name': volume.name,
                        'secret_provider_class': spc_name
                    })
    ...
    def get_associated_hpa(self, deployment) -> Optional[str]:
        ...
        hpas = self.autoscale_v1.list_namespaced_horizontal_pod_autoscaler(namespace=namespace)
...
class Serializer:
    ...
    def __init__(self, kubeclient: Any, v1_client: Any, networking_v1_client: Any, apps_client: Any, custom_client: Optional[Any] = None, autoscale_v1_client: Optional[Any] = None):
        """
        Initialize with Kubernetes API clients.
    ...
    def get_service(self, service_name: Optional[str], namespace: str) -> Optional[Dict[str, Any]]:
        if not service_name:
            return None
    ...
    def get_ingress(self, ingress_name: Optional[str], namespace: str) -> Optional[Dict[str, Any]]:
        if not ingress_name:
            return None
        ing = self.networking_v1.read_namespaced_ingress(name=ingress_name, namespace=namespace)
        if ing:
            return self._kubernetes_serializer(ing)
        return None

    def get_deployment(self, deployment_name: Optional[str], namespace: str) -> Optional[Dict[str, Any]]:
    ...
    def get_configmap(self, configmap_name: Optional[str], namespace: str) -> Optional[Dict[str, Any]]:
    ...
    def get_secret(self, secret_name: Optional[str], namespace: str) -> Optional[Dict[str, Any]]:
        if not secret_name:
            return None
    ...
    def get_secretprovider(self, name: Optional[str], namespace: str) -> Optional[Dict[str, Any]]:
    ...
    def get_hpa(self, hpa_name: Optional[str], namespace: str) -> Optional[Dict[str, Any]]:
        if not hpa_name:
            return None
...
class YAMLFormatter:
    ...
    def _apply_jq_filter(self, data: Union[Dict, List, Any], jq_filter: str) -> Dict[str, Any]:
        """
    ...
    def format_secret_provider_class(self, spc_data: Dict[str, Any], jq_filter: str) -> str:
        """
...
class FileManager:
    ...
    def write_yaml_file(self, file_path: Union[str, Path], data: Dict[str, Any], 
                       create_dirs: bool = True, yaml_formatter: Optional[YAMLFormatter] = None) -> Path:
        """
    ...
    def write_multiple_yaml_files(self, file_data_map: Dict[Union[str, Path], Dict[str, Any]], 
                                 base_directory: Optional[Union[str, Path]] = None,
                                 create_dirs: bool = True, 
                                 yaml_formatter: Optional[YAMLFormatter] = None) -> List[Path]:
        """
        ...
        try:
            ...
            if failed_files:
                failure_details = ", ".join([f"{path}: {error}" for path, error in failed_files])
                raise RuntimeError(f"Failed to write {len(failed_files)} files: {failure_details}")
    ...
    def create_resource_directory_structure(self, base_path: Union[str, Path], 
                                          namespace: str, resource_types: List[str]) -> Dict[str, Path]:
        """
        ...
        try:
            ...
            self.logger.info(f"Created directory structure for namespace '{namespace}' with {len(directory_map)} resource types")
    ...
    def create_cluster_backup_structure(self, base_path: Union[str, Path], 
                                       cluster_config: Dict[str, Dict[str, List[str]]]) -> Dict[str, Dict[str, Dict[str, Path]]]:
        """
        ...
        try:
            ...
            for cluster_name, namespaces in cluster_config.items():
                ...
                if not isinstance(namespaces, dict):
                    raise ValueError(f"Namespaces for cluster '{cluster_name}' must be a dictionary")
                ...
                for namespace, applications in namespaces.items():
                    if not namespace.strip():
                        self.logger.warning(f"Skipping empty namespace in cluster '{cluster_name}'")
                        continue
                        
                    if not isinstance(applications, list):
                        raise ValueError(f"Applications for namespace '{namespace}' must be a list")
                    ...
                    for application in applications:
                        if not application.strip():
                            self.logger.warning(f"Skipping empty application name in {cluster_name}/{namespace}")
                            continue
                            
                        app_path = namespace_path / application
                        created_dir = self.create_directory(app_path)
                        directory_structure[cluster_name][namespace][application] = created_dir
                        
                        self.logger.debug(f"Created application directory: {created_dir}")
            
            total_dirs = sum(
                len(apps) for cluster in directory_structure.values() 
                for apps in cluster.values()
            )
            self.logger.info(f"Created cluster backup structure with {len(directory_structure)} clusters, {total_dirs} application directories")
    ...
    def write_application_resources(self, base_path: Union[str, Path], cluster_name: str, 
                                  namespace: str, application: str, 
        ...
        try:
            if not all([cluster_name.strip(), namespace.strip(), application.strip()]):
                raise ValueError("cluster_name, namespace, and application cannot be empty")
            if not resources:
            ...
            self.logger.info(f"Wrote {len(written_files)} resource files for {cluster_name}/{namespace}/{application}")
            return written_files
    ...
    def create_backup_from_config_file(self, base_path: Union[str, Path], 
                                     config_file_path: Union[str, Path]) -> Dict[str, Dict[str, Dict[str, Path]]]:
        """
        ...
        try:
            ...
            if not isinstance(config_data, dict) or 'clusters' not in config_data:
                raise ValueError("Config file must contain a 'clusters' key with cluster definitions")
